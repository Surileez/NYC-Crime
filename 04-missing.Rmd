...
data<-read_csv('NYPD_Arrests_Data__Historic_.csv')
data<-data%>%mutate(ARREST_DATE=as.Date(data$ARREST_DATE,format='%m/%d/%Y'))
```
```{r}
data1<-data%>%filter(year(ARREST_DATE)<2010)
plot_missing(data1,angle=45,title='missing patterns for year before 2010')
data2<-data%>%filter(year(ARREST_DATE)<2015&year(ARREST_DATE)>=2010)
plot_missing(data2,angle=45,title='missing patterns for year between 2010-2014')
data3<-data%>%filter(year(ARREST_DATE)<=2019&year(ARREST_DATE)>=2015)
plot_missing(data3,angle=45,title='missing patterns for year between 2015-2019')
data4<-data%>%filter(year(ARREST_DATE)>=2020)
plot_missing(data4,angle=45,title='missing patterns for year after 2020')
```



```{r fig.width=10,fig.height=12}
nyc<-data%>%filter(year(ARREST_DATE)==2020&month(ARREST_DATE)=12)
tidynyc <- nyc %>% 
